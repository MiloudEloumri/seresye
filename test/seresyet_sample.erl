...
%%%
%%%-------------------------------------------------------------------
%%% @doc seresyet_sample eunit test.
%%%
%%% @version 0.0.5.
...
rule(Engine, {hello, world}, {ciao, X}, {X, 10}) ->
    seresye_engine:set_client_state(Engine, [rule | seresye_engine:get_client_state(Engine)]).
...
start() ->
    Engine0 =
        seresye_engine:add_rule(
            seresye_engine:new([]), {?MODULE, rule2}),
    Engine1 =
    ...
    Engine4 = seresye_engine:add_rules(Engine3, [{?MODULE, rule}, {?MODULE, rule1}]),

    Engine5 = seresye_engine:retract(Engine4, {test}),
    State =
        seresye_engine:get_client_state(
            seresye_engine:assert(Engine5, {test})),
    ?assertMatch([rule1b, rule1b, rule1a, rule3, rule3], State).
