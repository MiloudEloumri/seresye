...
%%%
%%%-------------------------------------------------------------------
%%% @doc Cannibals and Missionaries Problem
%%%
...
-export([shore_1_move/3, shore_2_move/3, cannibals_eat_missionaries/2, circular_path/3,
...
shore_1_move(Engine0,
    ...
    lists:foldl(fun(Missionaries, Engine1) ->
                   ...
                   lists:foldl(fun(Cannibals, Engine2) ->
                                  NewNode =
...
cannibals_eat_missionaries(Engine0,
                           #status{'shore-1-missionaries' = S1M,
                               ...
                               Node) ->
    if (S2C > S2M) and (S2M =/= 0) or (S1C > S1M) and (S1M =/= 0) ->
...
recognize_solution(Engine0,
                   #status{'shore-2-missionaries' = M, 'shore-2-cannibals' = C} = Node) ->
    if (M == ?INITIAL_MISSIONARIES) and (C == ?INITIAL_CANNIBALS) ->
...
verify_result(Node = #status{'shore-2-missionaries' = S2M, 'shore-2-cannibals' = S2C}) ->
