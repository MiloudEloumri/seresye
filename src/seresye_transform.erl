...
-record(state, {rules = [], rule_functions = [], exports = [], options}).
-record(context, {module, function, arity, file, options}).

parse_transform(Forms, Options) ->
    #context{file = File} = parse_trans:initial_context(Forms, Options),
    case erl_lint:module(Forms,
                         File,
    ...
    of
        ...
        _ ->
            ...
            lists:foldl(fun({Fun, Arity}, Acc) -> parse_trans:export_function(Fun, Arity, Acc) end,
                        Result0,
                        State#state.rule_functions)
    end.

do_transform(attribute,
             {attribute, _, export, Exports} = Attr,
             _Context,
             #state{} = State) ->
    {Attr, true, State#state{exports = State#state.exports ++ Exports}};
do_transform(attribute,
             {attribute, _, rule, Rule} = Attr,
             _Context,
             #state{rules = Rs} = State) ->
    {Attr, false, State#state{rules = [rule_name(Rule) | Rs]}};
do_transform(attribute,
             {attribute, _, rules, Rules0} = Attr,
             _Context,
             #state{rules = Rs} = State) ->
    Rules = [rule_name(R) || R <- Rules0],
    {Attr, false, State#state{rules = Rules ++ Rs}};
do_transform(function,
             {function, _, Fun, Arity, _Cs} = Form,
             _Context,
             #state{rules = Rules, rule_functions = RFuns} = State) ->
    case lists:member(Fun, Rules) of
        ...
        true ->
            {Form,
             true,
             State#state{rule_functions = [{Fun, Arity} | RFuns] -- State#state.exports}}
    end;
