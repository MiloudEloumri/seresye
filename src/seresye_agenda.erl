...
-export([new/1, add_activation/3, add_activation/4, breadth_order/2, clear_agenda/1,
         delete_activation/2, delete_rule/2, depth_order/2, fifo_order/2, get_activation/2,
         get_activation_from_name/2, get_activation_salience/2, get_first_activation/1,
         get_number_of_activations/1, get_rules_fired/1, get_strategy/1, pop_rule/1,
         set_activation_salience/3, set_rule_salience/3, run_hook/3, set_strategy/2]).

-include("internal.hrl").

-record(agenda, {rules_fired, strategy, rule_list, exec_state, id, pending_actions}).

%%====================================================================
%% External functions
%%====================================================================
new(Seresye) ->
    Seresye#seresye{agenda =
                        #agenda{rules_fired = 0,
                                strategy = depth,
                                rule_list = [],
                                exec_state = nil,
                                pending_actions = [],
...
set_strategy(EngineState = #seresye{agenda = Agenda0 = #agenda{rule_list = RuleList0}},
             NewStrategy) ->
    RuleList1 =
        case NewStrategy of
            depth ->
            ...
            breadth ->
                lists:sort(fun breadth_order/2, RuleList0);
            fifo ->
                lists:sort(fun fifo_order/2, RuleList0);
            _ ->
                erlang:throw({seresye, {invalid_strategy, NewStrategy}})
        end,
    EngineState#seresye{agenda =
                            Agenda0#agenda{strategy = NewStrategy, rule_list = RuleList1}}.
...
delete_activation(EngineState, []) ->
    EngineState;
delete_activation(EngineState =
                      #seresye{agenda = Agenda0 = #agenda{rule_list = RuleList0}},
                  Id)
    when not is_list(Id) ->
    EngineState#seresye{agenda =
                            Agenda0#agenda{rule_list = lists:keydelete(Id, 4, RuleList0)}};
...
delete_rule(EngineState = #seresye{agenda = Agenda0 = #agenda{rule_list = RuleList0}},
            Rule) ->
    ActList = proplists:lookup_all(Rule, RuleList0),
    RuleList1 = lists:foldl(fun(X, R1) -> lists:delete(X, R1) end, RuleList0, ActList),
...
get_activation_salience(#seresye{agenda = #agenda{rule_list = RuleList0, id = NextId}},
                        Id) ->
    case NextId < Id of
        true ->
            false;
...
set_activation_salience(EngineState =
                            #seresye{agenda =
                                         Agenda0 =
                                             #agenda{rule_list = RuleList0, strategy = Strategy}},
                        Id,
                        NewSalience)
    ...
    RuleList2 =
        case lists:keysearch(Id, 4, RuleList0) of
            {value, {Rule, Args, _Salience, Id}} ->
                RuleList1 = lists:keyreplace(Id, 4, RuleList0, {Rule, Args, NewSalience, Id}),
                order(RuleList1, Strategy);
            false ->
                RuleList0
        end,
    EngineState#seresye{agenda = Agenda0#agenda{rule_list = RuleList2}};
set_activation_salience(_EngineState, Id, NewSalience) when not is_list(Id) ->
    erlang:throw({seresye, {invalid_salience, NewSalience}});
set_activation_salience(EngineState0, [Id | OtherId], NewSalience) ->
...
%% returns the modified agenda
set_rule_salience(EngineState0 = #seresye{agenda = #agenda{rule_list = RuleList0}},
                  Rule,
                  NewSalience) ->
    ActList = proplists:lookup_all(Rule, RuleList0),
    IdList = [Id || {_, _, _, Id} <- ActList],
    set_activation_salience(EngineState0, IdList, NewSalience).

%%====================================================================
%% Internal functions
%%====================================================================
execute_pending(EngineState0 =
                    #seresye{agenda = Agenda0 = #agenda{pending_actions = [PA | Rest]}}) ->
...
after_activation_schedule(EngineState0 =
    ...
    {ExecState1, RuleList1, PA1, RF1} =
        if ExecState0 == active ->
               {ExecState0, RuleList0, PA0, RF0};
           true ->
               {RuleToExecute, RL} = pop_rule(RuleList0),
               if RuleToExecute == false ->
                      {nil, RL, PA0, RF0};
                  true ->
                      {active,
                       RL,
                       PA0 ++ [fun(EngineState) -> exec(EngineState, RuleToExecute) end],
                       RF0 + 1}
               end
        end,
    EngineState0#seresye{agenda =
                             Agenda0#agenda{exec_state = ExecState1,
                                            rule_list = RuleList1,
                                            pending_actions = PA1,
                                            rules_fired = RF1}}.
...
depth_add(RuleList, Rule, Args, Salience, Id) ->
    {L1, L2} = lists:splitwith(fun({_, _, S, _}) -> S > Salience end, RuleList),
...
order(ActionList, Strategy) ->
    case Strategy of
        ...
        depth ->
            lists:sort(fun depth_order/2, ActionList);
        fifo ->
            lists:sort(fun fifo_order/2, ActionList);
        _Other ->
            ActionList
    end.

%% @doc define when an Act1 comes before Act2 in breadth strategy
breadth_order(Act1, Act2) when element(3, Act1) > element(3, Act2) ->
    true;
breadth_order(Act1, Act2) when element(3, Act1) < element(3, Act2) ->
    false;
breadth_order(Act1, Act2) when element(4, Act1) > element(4, Act2) ->
    false;
breadth_order(_Act1, _Act2) ->
    true.

%% @doc define when an Act1 comes before Act2 in depth strategy
depth_order(Act1, Act2) when element(3, Act1) > element(3, Act2) ->
    true;
depth_order(Act1, Act2) when element(3, Act1) < element(3, Act2) ->
    false;
depth_order(Act1, Act2) when element(4, Act1) < element(4, Act2) ->
    false;
depth_order(_Act1, _Act2) ->
    true.

%% @doc define when an Act1 comes before Act2 in fifo strategy
fifo_order(Act1, Act2) when element(4, Act1) > element(4, Act2) ->
    false;
fifo_order(_Act1, _Act2) ->
    true.
    ...
    EngineState2 =
        case catch execute_rule(EngineState0, R) of
            {'EXIT', {function_clause, [{Mod, Fun, _} | _]}} ->
                EngineState0;
            {'EXIT', {function_clause, [{Mod, Fun, _, _Location} | _]}} ->
                EngineState0;
            {'EXIT', Reason} ->
                erlang:throw({seresye, {rule_execution, [R, Reason]}});
            EngineState1 ->
                EngineState1
        end,
