...
-export([child/3, children/2, get_beta/1, get_id/1, get_key/1, get_last_insert/1,
         get_node/2, get_parent/2, get_root/1, have_child/1, insert/4, is_present/2, is_root/1,
         keysearch/2, lookup_all/2, new/0, refresh/2, remove_child/3, remove_node/2, set_child/3,
...
insert(Key, Value, ParentNode, List) ->
    {ParentKey, ParentValue, Children, Parent, Pos} = ParentNode,
    Next = length(List) + 1,
    Node = {Key, Value, [], Pos, Next},
    List1 = List ++ [Node],
    Children1 = Children ++ [Next],
    ParentNode1 = {ParentKey, ParentValue, Children1, Parent, Pos},
...
remove_child(Child, ParentNode, List) ->
    ...
    NewNode = {Key, Value, Children -- [ChildId], Parent, Pos},
    lists:keyreplace(Pos, 5, List, NewNode).
...
update_list(List, [Node | T], N) ->
    ...
    {NewParent, List1} =
        case Parent > N of
            ...
            false ->
                NewParent0 = Parent,
                {Key1, Value1, Children1, Parent1, Pos1} = lists:nth(NewParent0, List),
                ParentNode = {Key1, Value1, (Children1 -- [Pos]) ++ [NewPos], Parent1, Pos1},
                {NewParent0, lists:keyreplace(Parent, 5, List, ParentNode)}
        end,
    NewChildren = [update_list_1(V1, N) || V1 <- Children],
    NewNode = {Key, Value, NewChildren, NewParent, NewPos},
    List2 = List1 ++ [NewNode],
