...
-export([new/0, new/1, serialize/1, restore/1, cleanup/1, add_hook/3, set_hooks/2,
         get_fired_rule/1, add_rules/2, add_rule/2, add_rule/3, assert/2, get_kb/1,
         get_rules_fired/1, get_client_state/1, set_client_state/2, query_kb/2, remove_rule/2,
...
new(ClientState) ->
    seresye_agenda:new(#seresye{kb = [],
                                alfa = [],
                                join = seresye_tree_list:new(),
                                pending_actions = [],
                                client_state = ClientState}).

add_hook(#seresye{hooks = Hooks} = EngineState, Hook, Fun)
    when is_atom(Hook), is_function(Fun) ->
...
cleanup(#seresye{alfa = Alfa0, join = Join0}) ->
    ...
    ++ [begin
        ...
        || {{Tab, _}, _, _, _, _} <- Join0, is_integer(Tab)].

restore(#seresye{alfa = Alfa0, join = Join0} = Engine) ->
    TabCache = ets:new(tab_cache, []),
    Alfa = [{Cond, restore_tab(TabCache, Tab), Alfa_fun} || {Cond, Tab, Alfa_fun} <- Alfa0],
    Join =
        [{case Key of
...
serialize(#seresye{alfa = Alfa0, join = Join0} = Engine) ->
    Alfa = [{Cond, serialize_tab(Tab), Alfa_fun} || {Cond, Tab, Alfa_fun} <- Alfa0],
    Join =
        [{case Key of
        ...
        [] ->
            NewTab =
                ets:new(
                    ...
                    [proplists:get_value(Opt, Info) || Opt <- [type, protection]]
                    ++ [{Opt, proplists:get_value(Opt, Info)} || Opt <- [keypos]]),
...
assert(EngineState0, Facts) when is_list(Facts) ->
    lists:foldl(fun(Fact, EngineState1) -> assert(EngineState1, Fact) end,
                EngineState0,
                Facts);
assert(EngineState = #seresye{kb = Kb, alfa = Alfa}, Fact) when is_tuple(Fact) ->
    execute_pending(case lists:member(Fact, Kb) of
                        false ->
                            ...
                            check_cond(EngineState#seresye{kb = Kb1}, Alfa, {Fact, plus});
                        true ->
                            EngineState
                    end).
...
retract(EngineState0, Facts) when is_list(Facts) ->
    lists:foldl(fun(Fact, EngineState1) -> retract(EngineState1, Fact) end,
                EngineState0,
                Facts);
retract(EngineState = #seresye{kb = Kb, alfa = Alfa}, Fact) when is_tuple(Fact) ->
    execute_pending(case lists:member(Fact, Kb) of
                        true ->
                            ...
                            check_cond(EngineState#seresye{kb = Kb1}, Alfa, {Fact, minus});
                        false ->
                            EngineState
                    end).
...
add_rules(EngineState0, RulesList) when is_list(RulesList) ->
    lists:foldl(fun(Rule, EngineState1) -> add_rule(EngineState1, Rule) end,
                EngineState0,
                RulesList);
...
add_rule(EngineState0, {Module, Fun}, ClauseID, Salience) ->
    ...
    case get_conds(Fun, ClauseID, AST) of
        error ->
            erlang:throw({seresye, {error_extracting_conditions, Fun}});
        CondsList ->
            execute_pending(lists:foldl(fun(X, EngineState1) ->
                                           case X of
                                               {error, Msg} ->
...
query_kb(EngineState, Pattern) when is_tuple(Pattern) ->
    ...
    MatchFunction =
        fun(P) ->
           ...
           MatchResult = lists:foldr(fun(X, Y) -> X and Y end, true, FunEval),
           MatchResult
        end,
    KB = [X || X <- get_kb(EngineState), size(X) == PatternSize],
...
add_rule__(EngineState0 = #seresye{join = Join}, Rule, {PConds, NConds})
    when is_tuple(Rule) ->
    Root = seresye_tree_list:get_root(Join),
    case NConds of
        [] ->
            make_struct(EngineState0, Rule, PConds, [], Root, nil);
        _ ->
            {EngineState1, Np_node} = make_struct(EngineState0, Rule, NConds, [], Root, ng),
            Root1 = seresye_tree_list:refresh(Root, EngineState1#seresye.join),
            make_struct(EngineState1, Rule, PConds, [], Root1, {Np_node, NConds})
    end.

%% @doc initializes the memory with a new alpha-present the facts
%% In the Knowledge Base that satisfy the condition Cond
initialize_alfa(_Engine, _, _, []) ->
    nil;
...
prepare_match_alpha_fun(Cond) ->
    Expr =
        [{'fun',
          1,
          {clauses,
           [{clause, 1, [{match, 1, Cond, {var, 1, 'X__x__X'}}], [], [{var, 1, 'X__x__X'}]},
...
get_abstract_code(Module) ->
    ...
    of
        {error, beam_lib, {file_error, File, enoent}} ->
            erlang:throw({seresye, {unable_to_find_file, Module, File}});
        {ok, {Module, [{abstract_code, no_abstract_code}]}} ->
            erlang:throw({seresye,
                          {no_abstract_code, Module, "module must be compiled with +debug_info"}});
...
get_conds(FunName, ClauseID, AST) ->
    ...
    case search_fun(FunName, AST) of
        {error, Reason} ->
            erlang:throw({seresye, {error_parsing_forms, FunName, Reason}});
        {ok, CL} ->
            if ClauseID > 0 ->
                   [read_clause(FunName, ClauseID, lists:nth(ClauseID, CL), Records, AST)];
               true ->
                   [read_clause(FunName, ClauseID, Clause, Records, AST) || Clause <- CL]
            end
    end.

get_records([], Acc) ->
    Acc;
get_records([{attribute, _, record, {RecordName, RecordFields}} | Tail], Acc) ->
    NewAcc = [{RecordName, get_record_fields(RecordFields, [])} | Acc],
    get_records(Tail, NewAcc);
get_records([_ | Tail], Acc) ->
    get_records(Tail, Acc).

get_record_fields([], Acc) ->
    lists:reverse(Acc);
get_record_fields([{record_field, _, {atom, _, FieldName}, {nil, _}} | Tail], Acc) ->
    NewAcc = [{FieldName, {nil, []}} | Acc],
    get_record_fields(Tail, NewAcc);
get_record_fields([{record_field, _, {atom, _, FieldName}, {Type, _, DefaultValue}}
                   | Tail],
                  Acc) ->
    NewAcc = [{FieldName, {Type, DefaultValue}} | Acc],
    get_record_fields(Tail, NewAcc);
get_record_fields([{record_field, _, {atom, _, FieldName}} | Tail], Acc) ->
...
extract_parameters([{match, _, Condition, {var, _, _}} | Tail], RecordList, Acc) ->
...
extract_parameters([{match, _, {var, _, _}, {record, _, _, _} = R} | Tail],
                   ...
                   Acc) ->
    extract_parameters([R | Tail], RecordList, Acc);
extract_parameters([{match, _, {var, _, _}, Condition} | Tail], RecordList, Acc) ->
    extract_parameters(Tail, RecordList, [remove_line_numbers(Condition) | Acc]);
extract_parameters([{record, _, RecordName, Condition} | Tail], RecordList, Acc) ->
    RecordDefinition = get_record_def(RecordName, RecordList),
    RecordDefaults = make_record_default(RecordDefinition, []),
    Pattern =
        {tuple,
         0,
         [{atom, 0, RecordName} | make_record_pattern(Condition,
                                                      RecordDefaults,
                                                      RecordDefinition)]},
    extract_parameters(Tail, RecordList, [Pattern | Acc]);
extract_parameters([{tuple, L, Elements} | Tail], RecordList, Acc) ->
    Pattern = {tuple, L, extract_parameters(Elements, RecordList, [])},
    extract_parameters(Tail, RecordList, [Pattern | Acc]);
...
remove_line_numbers({Type, L}) when is_integer(L), is_atom(Type) ->
    {Type, 0};
...
remove_line_numbers({Type, L, Value}) when is_integer(L), is_atom(Type) ->
    {Type, 0, Value};
remove_line_numbers({Type, L, C1, C2}) when is_integer(L), is_atom(Type) ->
...
make_record_pattern([], Pattern, _RecordDefinition) ->
    Pattern;
make_record_pattern([{record_field, _, {atom, _, FieldName}, MatchValue} | Tail],
                    Pattern,
                    RecordDefinition) ->
    ...
    NewPattern =
        ...
        ++ [MatchValue]
        ++ lists:sublist(Pattern, FieldPosition + 1, length(Pattern) - FieldPosition),
...
make_struct(EngineState0 = #seresye{join = Join}, _Rule, [], _P, Cur_node, ng) ->
    ...
    {Node, {Join2, EngineState1}} =
        case seresye_tree_list:child(Key, Cur_node, Join) of
            false ->
                Value = [],
                {Node0, Join1} = seresye_tree_list:insert(Key, Value, Cur_node, Join),
                {Node0, update_new_node(EngineState0, Node0, Cur_node, Join1)};
            Node0 ->
                {Node0, {Join, EngineState0}}
        end,
...
make_struct(EngineState0 = #seresye{join = Join}, Rule, [], _P, Cur_node, nil) ->
    ...
    {Join2, EngineState1} =
        case seresye_tree_list:child(Key, Cur_node, Join) of
            false ->
                Value = [],
                {Node, Join1} = seresye_tree_list:insert(Key, Value, Cur_node, Join),
                update_new_node(EngineState0, Node, Cur_node, Join1);
            Node ->
                {Fun, _Salience} = Rule,
                Key1 = element(1, Node),
...
make_struct(EngineState0 = #seresye{join = Join}, Rule, [], P, Cur_node, {Nod, NConds}) ->
    ...
    {Cur_node1, Join1, EngineState1} =
        make_join_node(EngineState0, Join, {old, {n_node, Id}}, NConds, P, Cur_node),
    Nod1 = seresye_tree_list:refresh(Nod, Join1),
    Join2 = seresye_tree_list:set_child(Cur_node1, Nod1, Join1),
    Key = {p_node, Rule},
    {Join4, EngineState2} =
        case seresye_tree_list:child(Key, Cur_node1, Join2) of
            false ->
                Value = [],
                {Node, Join3} = seresye_tree_list:insert(Key, Value, Cur_node1, Join2),
                Cur_node2 = seresye_tree_list:refresh(Cur_node1, Join3),
                update_new_node(EngineState1, Node, Cur_node2, Join3);
            Node ->
                {Fun, _Salience} = Rule,
                Key1 = element(1, Node),
                Sal = element(2, element(2, Key1)),
...
make_struct(EngineState0 = #seresye{join = Join}, Rule, [Cond | T], P, Cur_node, Nod) ->
    ...
    {Cur_node1, Join1, EngineState1} =
        make_join_node(EngineState0, Join, Tab, Cond, P, Cur_node),
    P1 = P ++ [Cond],
    make_struct(EngineState1#seresye{alfa = Alfa1, join = Join1},
                Rule,
                T,
                P1,
                Cur_node1,
...
add_alfa(#seresye{kb = Kb, alfa = Alfa} = Engine, Cond) ->
    case is_present(Cond, Alfa) of
        false ->
            ...
            Fun = [{'fun', 1, {clauses, [{clause, 1, [Cond], [], [{atom, 1, true}]}]}}],
            Alfa_fun = eval(Fun),
...
remove_prod(EngineState0 = #seresye{join = Join}, Fun) ->
    case seresye_tree_list:keysearch({p_node, Fun}, Join) of
        ...
        Node ->
            ...
            Parent_node1 = seresye_tree_list:refresh(Parent_node, Join1),
            EngineState1 = remove_nodes(Parent_node1, EngineState0#seresye{join = Join1}),
                ...
                false ->
                    ...
                    case First of
                        {n_node, IdNp_node} ->
                            ...
                            Join2 = seresye_tree_list:remove_child(Node, Np_node, Join1),
                            Np_node1 = seresye_tree_list:refresh(Np_node, Join2),
                            EngineState1 =
                                remove_nodes(Np_node1, EngineState0#seresye{join = Join2}),
                            ...
                            Parent_node2 = seresye_tree_list:keysearch(ParentKey, Join3),
                            remove_nodes(Parent_node2, EngineState1);
                        np_node ->
                            remove_nodes(Parent_node1, EngineState0#seresye{join = Join1});
                        Tab ->
                            Alfa1 =
                                case seresye_tree_list:is_present(Tab, Join1) of
                                    false ->
                                        ets:delete(Tab),
                            ...
                            remove_nodes(Parent_node1,
                                         EngineState0#seresye{alfa = Alfa1, join = Join1})
                    end;
                true ->
                    EngineState0
            end;
...
is_present(Cond, [{C1, Tab, _Alfa_fun} | Other_cond]) ->
    case same_cond(Cond, C1) of
        true ->
            {true, Tab};
        false ->
            is_present(Cond, Other_cond)
    end.

same_cond(Cond, Cond) ->
    true;
same_cond(_Cond1, _Cond2) ->
    false.
...
prepare_fun(_Cond, []) ->
    [{atom, 1, nil}];
prepare_fun(Cond0, Cond1) when not is_list(Cond0), is_list(Cond1) ->
    [{'fun',
      1,
      {clauses, [{clause, 1, [Cond0, prepare_rest(Cond1)], [], [{atom, 1, true}]}]}}];
prepare_fun(Cond0, Cond1) when is_list(Cond0), is_list(Cond1) ->
    [{'fun',
      1,
      {clauses,
       [{clause, 1, [prepare_rest(Cond0), prepare_rest(Cond1)], [], [{atom, 1, true}]}]}}].
...
update_agenda(EngineState, [], _Rule) ->
    EngineState;
...
update_new_node(EngineState0, _Node, _Parent_node, Join, []) ->
    {Join, EngineState0};
update_new_node(EngineState0, Node, Parent_node, Join, [Fact | Other_fact]) ->
...
update_from_n_node(EngineState0, Parent_node, [Tok | OtherTok], Join) ->
    {Join1, EngineState1} = left_act(EngineState0, {Tok, plus}, [Parent_node], Join),
    update_from_n_node(Parent_node, OtherTok, Join1, EngineState1).
...
signal(EngineState0 = #seresye{pending_actions = PAList}, Token, Sign, {Fun, Salience}) ->
    NewPA =
        case Sign of
            plus ->
                fun(EngineState1) ->
                   seresye_agenda:add_activation(EngineState1, Fun, Token, Salience)
                end;
            minus ->
                ActivationId = seresye_agenda:get_activation(EngineState0, {Fun, Token}),
                fun(EngineState1) -> seresye_agenda:delete_activation(EngineState1, ActivationId)
                end
        end,
...
%%====================================================================
check_cond(EngineState, [], {_Fact, _Sign}) ->
    EngineState;
check_cond(EngineState0, [{_C1, Tab, Alfa_fun} | T], {Fact, Sign}) ->
...
pass_fact(EngineState0 = #seresye{join = Join}, Tab, {Fact, Sign}) ->
    ...
    {Join1, EngineState1} = propagate(EngineState0, Succ_node_list, {Fact, Sign}, Join),
...
propagate(EngineState0, [Join_node | T], {Fact, Sign}, Join) ->
    ...
    {Join1, EngineState1} =
        case seresye_tree_list:get_key(Join_node1) of
            {{n_node, _}, _} ->
                propagate_nnode(EngineState0, Join_node1, Tok_list, Sign, Join);
            {_Tab, _Fun} ->
                Children_list = seresye_tree_list:children(Join_node1, Join),
                pass_tok(EngineState0, Tok_list, Children_list, Join)
        end,
...
propagate_nnode(EngineState0, Join_node, Tok_list, Sign, Join) ->
    Children_list = seresye_tree_list:children(Join_node, Join),
    Toks = [make_toks(Tok, Sign) || Tok <- Tok_list],
    case Sign of
        plus ->
            pass_tok(EngineState0, Toks, Children_list, Join);
        minus ->
            {{n_node, IdNp_node}, Join_fun} = seresye_tree_list:get_key(Join_node),
...
left_act(EngineState0, {Token, Sign}, [Join_node | T], Join) ->
    ...
    Join1 = seresye_tree_list:update_beta(Beta1, Join_node, Join),
    case seresye_tree_list:get_key(Join_node) of
        {p_node, Rule} ->
            left_act(signal(EngineState0, Token, Sign, Rule), {Token, Sign}, T, Join1);
        {{n_node, IdNp_node}, Join_fun} ->
            left_act_nnode(EngineState0,
                           {Token, Sign},
                           IdNp_node,
                           Join_fun,
                           [Join_node | T],
                           Join1);
        {np_node, nil} ->
            Children_list = seresye_tree_list:children(Join_node, Join1),
            {Join2, EngineState1} = propagate(EngineState0, Children_list, {Token, Sign}, Join1),
            left_act(EngineState1, {Token, Sign}, T, Join2);
        {Tab, Join_fun} ->
            Alfa_mem = ets:tab2list(Tab),
            Tok_list = join_left({Token, Sign}, Alfa_mem, Join_fun),
            Children_list = seresye_tree_list:children(Join_node, Join1),
            {Join2, EngineState1} = pass_tok(EngineState0, Tok_list, Children_list, Join1),
            left_act(EngineState1, {Token, Sign}, T, Join2)
    end.
...
left_act_nnode(EngineState0, {Token, Sign}, IdNp_node, Join_fun, [Join_node | T], Join) ->
    ...
    case Tok_list of
        [] ->
            Children_list = seresye_tree_list:children(Join_node, Join),
            {Join1, EngineState1} = pass_tok(EngineState0, [{Token, Sign}], Children_list, Join),
            left_act(EngineState1, {Token, Sign}, T, Join1);
        Tok_list ->
            left_act(EngineState0, {Token, Sign}, T, Join)
    end.

test_nnode(EngineState0, [], _, _, _, Join) ->
    {Join, EngineState0};
test_nnode(EngineState0, [Tok | OtherTok], IdNpNode, Join_fun, Join_node, Join) ->
    {Join1, EngineState1} =
        left_act_nnode(EngineState0, Tok, IdNpNode, Join_fun, [Join_node], Join),
    test_nnode(EngineState1, OtherTok, IdNpNode, Join_fun, Join_node, Join1).
...
new_join(EngineState0, J, Tab, Join_fun, Parent_node) ->
    Key = {Tab, Join_fun},
    Value = [],
    {Node, J1} = seresye_tree_list:insert(Key, Value, Parent_node, J),
